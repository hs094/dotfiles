...
# 1. Focused pane's foreground process matches Vim (or HERDR_NAV_PASSTHROUGH_RE):
#    forward the key into the pane and let the editor/app decide.
# 2. Otherwise, try `herdr pane focus --direction DIR --pane $HERDR_PANE_ID`
#    (the pane is targeted explicitly because `--current` resolves to the
#    server's globally focused pane, which is not necessarily this one):
...
up)    key="ctrl+k" ;;
right) key="ctrl+l" ;;
*) echo "vim-herdr-navigation: unknown direction: $dir" >&2; exit 2 ;;
esac
...
# e.g. HERDR_NAV_PASSTHROUGH_RE='^(lazygit|k9s|vi-sql)$'
...
# Target the pane explicitly: `--current` resolves to the server's globally
# focused pane, which is not necessarily the one we are in.
if [ -n "$pane" ]; then
  focus_output="$("$herdr" pane focus --direction "$dir" --pane "$pane" 2>/dev/null)" && exit_code=0 || exit_code=$?
else
